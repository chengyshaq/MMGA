...
import MMGA.model.hs_densenet as dn
import MMGA.model.hs_resnet as rn
...
def evaluation():
    ...
    if args.ood_data == "imagenet":
        if args.dataset == "nus-wide":
            ...
            out_test_data = torchvision.datasets.ImageFolder(ood_root, transform=img_transform)
            # print("class:", out_test_data.classes)
    ...
    if args.arch == "resnet101":
        ...
        model = dn.densenet121(pretrained=True) # HS-densenet
        clsfier = clssimp(1024, args.n_classes)
        

    model.load_state_dict(torch.load(args.load_model + args.arch + '.pth'))
    clsfier.load_state_dict(torch.load(args.load_model + args.arch + 'clsfier.pth'))
    print("model loaded!")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ood', type=str, default='energy',                                                                        
                        help='which measure to use odin|M|logit|energy|msp|prob|lof|isol|React_energy')
